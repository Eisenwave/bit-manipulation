...
constexpr decltype(auto) visit(F&& f, V&& v)
{
    ...
    constexpr Size alternatives = VV::alternatives;

    if constexpr (alternatives == 1) {
        return static_cast<F&&>(f)(::std::get<0>(static_cast<V&&>(v)));
    }
    else if constexpr (alternatives == 2) {
        switch (v.index()) {
    ...
    else if constexpr (alternatives <= 4) {
        switch (v.index()) {
            BIT_MANIPULATION_VISIT_CASE(0);
            BIT_MANIPULATION_VISIT_CASE(1);
    ...
    else if constexpr (alternatives <= 8) {
        switch (v.index()) {
            BIT_MANIPULATION_VISIT_CASE(0);
            BIT_MANIPULATION_VISIT_CASE(1);
            BIT_MANIPULATION_VISIT_CASE(2);
    ...
    else if constexpr (alternatives <= 32) {
        switch (v.index()) {
            BIT_MANIPULATION_VISIT_CASE(0);
            BIT_MANIPULATION_VISIT_CASE(1);
            BIT_MANIPULATION_VISIT_CASE(2);
            ...
            BIT_MANIPULATION_VISIT_CASE(25);
            BIT_MANIPULATION_VISIT_CASE(26);
            BIT_MANIPULATION_VISIT_CASE(27);
            BIT_MANIPULATION_VISIT_CASE(28);
            BIT_MANIPULATION_VISIT_CASE(29);
            BIT_MANIPULATION_VISIT_CASE(30);
            BIT_MANIPULATION_VISIT_CASE(31);
        }
        BIT_MANIPULATION_ASSERT_UNREACHABLE("impossible variant index");
    }
}
